...
public class ManagingController  {
    ...
    @PostMapping("/add")
    public ResponseEntity<BrigadeRecord> addBrigadeRecord(@RequestBody BrigadeRecord brigadeRecord) {
    ...
    public ResponseEntity<List<MinerPartResponse>> showManagingByBrigadeId(@PathVariable @RequestBody int id){
        return new ResponseEntity<>(managingService.showBrigadeRecordsByBrigadeId(id), HttpStatus.OK);
    }
